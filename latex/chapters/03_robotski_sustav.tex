...
Sedma os čini strukturu redundantnom u odnosu na šest stupnjeva slobode zadatka, pa
isti položaj i orijentaciju vrha alata ostvaruje veći broj konfiguracija.
Tehnički podaci manipulatora nalaze se u tablici \ref{tab:lbr}.
...
Time se izbjegava zazor koji bi pri guranju vrata dopustio da se kvaka pomiče unutar hvata.
...
prigušenjem 200 Ns/m i graničnom silom 600 N. Pri guranju vrata silu prenose djelotvorno samo
...
\subsection{Otpor gibanja i njegovo nasumično variranje}
\label{sec:otpor}
...
U tablici \ref{tab:otpor-nasumicnost} mogu se vidjeti rasponi iz kojih su birane vrijednosti otpora vrata:

\begin{table}[H]
    \centering
    \caption{Rasponi iz kojih se bira otpor vrata}
    \label{tab:otpor-nasumicnost}
    \begin{tabular}{@{}lcc@{}}
        \toprule
...
\subsection{Odometrija}
\label{sec:odometrija}

Prijeđeni put platforme ne računa se iz naređene brzine i vremena, nego se mjeri. Most prema
simulatoru objavljuje odometriju izvedenu iz stvarne poze članka platforme u sceni, pa se
podudara sa stvarnim stanjem. Budući da platforma u modelu nema kotače, nego je predstavljena
jednim kolizijskim tijelom, enkodera iz kojih bi se odometrija inače izvela nema.

Uporaba te veličine u upravljanju ipak ne narušava načelo da se stanje simulacije ne čita u
regulacijskoj petlji. Odometrija je senzorska veličina koju stvarni KMR iiwa posjeduje i
objavljuje kao dio isporučene programske podrške, za razliku od, primjerice, kuta zgloba vrata,
koji na stvarnom robotu ne bi bio dostupan. Razlika je u tome što je ovdje riječ o savršenoj
odometriji, bez proklizavanja i bez akumulirajućeg odstupanja, što stvarni enkoderi imaju.
...
Na slici \ref{fig:arch-klasicni} može se vidjeti dijagram arhitekture za klasični pristup.
